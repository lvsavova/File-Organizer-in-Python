...
def organize():
    ...
    if selected_type == "content" and content_entry.get() == "":
        showwarning('Ok', 'Please choose strings by which docx files are organized!')
        raise Exception('No strings by which to perform content re-organization ')
...
Radiobutton(root, text="By Extension", variable=v, value="extension",
            command=lambda: callback_rb("extension", "disabled")).pack(anchor=W)
...
w = Label(root, text="Type a string (strings, semicolon separated) by which *.docx files will be sorted:")
w.pack()
content_entry.pack()
